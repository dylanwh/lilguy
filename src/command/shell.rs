...
use crate::runtime::{self, Options, Runtime};
...
impl Shell {
    ...
    pub async fn run(self, context: &Context, runtime: Runtime) -> Result<(), Error> {
        runtime.start(Options { reload: false }).await?;
        let config = context.config.shell.clone();
        ...
        context.output.set_printer(printer.clone());
        ...
        context
            .tracker
            .spawn_blocking(move || read_loop(reedline, prompt_config, tx));
        context.tracker.spawn(eval_loop(
            context.token.clone(),
            rx,
            printer,
            highlighter,
...
impl LuaHighlighter {
    fn new(theme: LuaHighlighterConfig) -> Result<Self, Error> {
